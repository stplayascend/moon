...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const g = gameKey.toLowerCase().trim();
      ...
      const gameData = items[gameKey];
      let base = [];

      if (gameKey === 'fishitBoost') {
        if (c === 'gift') base = gameData?.giftPackages || [];
        if (c === 'login') base = gameData?.loginPackages || [];
      } else if (gameKey === 'robuxUsername') {
